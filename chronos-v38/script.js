...
const MUSIC_KEYS_MAP = ['CAS','LANA','BEEGEES','NIGHTRIDE','LOFI'];
const STREAMS = [
    { name:'CAS FM',       key:'CAS'       },
    { name:'LANA DEL REY', key:'LANA'      },
    { name:'BEE GEES',     key:'BEEGEES'   },
    { name:'NIGHTRIDE FM', key:'NIGHTRIDE' },
    { name:'LOFI HIP-HOP', key:'LOFI'      },
];
const QUOTES = [
...
window.onload = () => {
    ...
    updateTimerDisplay(); updateArc(1); updateSunrise(0); updateProgressBar(1);
    if (pilotName) {
        setText('pilotDisplay',pilotName); setText('pilotInline',pilotName);
        setText('pilotTag',pilotName);
        setText('systemStatus','STANDBY · PILOT: '+pilotName);
    }
    renderWisdom();
    addLog('SYSTEMS_ONLINE · V38');
...
function tick() {
    if (--secondsLeft <= 0) {
        secondsLeft = 0; updateTimerDisplay(); updateArc(0); updateSunrise(100); updateProgressBar(0);
        warpComplete(); return;
    }
    const p = secondsLeft / totalSeconds;
    updateTimerDisplay(); updateArc(p); updateSunrise((1-p)*100); updateProgressBar(p);
}
...
async function warpComplete() {
    ...
    // Auto-play music inline via the embedded stream player
    if (musicEnabled) {
        addLog('LAUNCHING STREAM: '+currentStreamName);
        setTimeout(() => playAutomatedMusic(), 800);
    }
}
...
function updateProgressBar(fraction) {
    const pb = getEl('progress-bar');
    if (pb) pb.style.width = (fraction*100) + '%';
}
...
function applyTheme(name,btn) {
    ...
    const sel = getEl('setup-theme'); if (sel) sel.value = name;
...
function selectStream(i,btn) {
    ...
    // Link to music_ytb.js key
    if (typeof setActiveMusicKey === 'function') setActiveMusicKey(STREAMS[i].key);
    addLog('STREAM_QUEUED: '+currentStreamName);
    // If music is already playing, switch stream live
...
function toggleMusic() {
    ...
    const el2=getEl('musicStatus2'); if (el2) el2.textContent=musicEnabled?'ON':'OFF';
    if (!musicEnabled && typeof stopMusic === 'function') stopMusic();
}
...
function initMission() {
    ...
    setText('pilotInline',pilot); setText('pilotTag',pilot);
    setText('systemStatus','STANDBY · PILOT: '+pilot);
...
function applyParams() {
    ...
    totalSeconds=dur*60; secondsLeft=totalSeconds;
    setText('intentDisplay',intent); updateTimerDisplay(); updateArc(1); updateProgressBar(1);
...
function saveAndQuit() {
    store.set('sv38_sessions',sessionCount); store.set('sv38_minutes',minuteCount); store.set('sv38_flights',flightHistory);
    addLog('FLIGHT_LOG_SAVED');
    // Reset to standby instead of closing (window.close() blocked in most browsers)
    setTimeout(()=>{ resetTimer(); showModal('setup-modal'); }, 1200);
}
...
function renderFlights() {
    ...
    el.innerHTML=flightHistory.slice(0,8).map(f=>
        `<div class="flt-entry"><span class="fn">#${f.num}</span><span>${f.dur}MIN</span><span>${f.pilot}</span><span style="opacity:0.5">${f.time}</span></div>`
    ).join('')||'<div style="opacity:0.5;font-size:0.55rem;padding:6px 0;letter-spacing:2px;">NO FLIGHTS YET</div>';
}
function addLog(msg) {
